...
public class AlunoService {
    public static void InserirAluno(Aluno a) throws SQLException {  
    	String query = "select max(id) as id from tb_usuario";
    	int id = 0;
        ...
        query = String.format("insert into tb_usuario (id,nome,ra,curso) values(%s,'%s','%s','%s')",id, a.getNome(), a.getRa(), a.getCurso());
        Dao.InsertUpdateDeleteCommand(query);
    }
   
    public static void AtualizarAluno(Aluno a) {
        String query = String.format("update tb_usuario set nome = '%s', ra = '%s', curso = '%s' where id = %s", a.getNome(), a.getRa(), a.getCurso(),a.getId());
        Dao.InsertUpdateDeleteCommand(query);
    }
    ...
    public static void ExibirAlunos() {
        String query= "select * from tb_usuario order by id";
